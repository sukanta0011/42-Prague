		...
		char *str)
{
	...
	{
		print_padding_char(' ', (width - precision));
		ft_putstr(str);
	...
	else if (fmt_spcfr->dot && precision >= len && width < precision)
	{
		...
		extend_num_left_padding(fmt_spcfr, pad, str);
	ft_putstr(fmt_spcfr->var.str);
}

void	extend_num_left_padding(t_fmt_specifier *fmt_spcfr, char pad, char *str)
{
	t_uint	len;
	t_uint	width;
		...
		char *str)
{
	...
	if (fmt_spcfr->dot && precision > len && width >= precision)
	{
		...
		print_padding_char('0', (precision - len));
		ft_putstr(fmt_spcfr->var.str);
		print_padding_char(pad, (width - precision));
	}
	else if (fmt_spcfr->dot && precision > len && width < precision)
		use_num_left_padding(fmt_spcfr, '0', str);
	else
	{
		ft_putstr(str);
